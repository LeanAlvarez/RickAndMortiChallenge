...
class RickAndMortyDataAdapter (private val results: List<RickAndMortyModel>, private val itemClickListener: OnItemClickListener)  : RecyclerView.Adapter<BaseViewHolder<*>>() {
    ...
    private inner class ItemViewHolder(val binding: ItemListDataBinding, val context: Context): BaseViewHolder<RickAndMortyModel>(binding.root){
        ...
        override fun bind(item: RickAndMortyModel) {
            ...
            if(item.status == "Alive"){
                ...
                //binding.statusLive.setTextColor(R.color.statusUknown)
            }
